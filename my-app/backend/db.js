...
app.post('/login',(req,res,next) => {
    ...
    app.post('/findItem',(req,res,next)=>{
        ...
        User.find({_id:req.body.id},{menu:1,_id:0}).then(result=>{
    ...
    app.post('/removeItem',(req,res,next)=>{
        ...
        User.updateOne(
            {_id:req.body.userid },
            { $pull: { menu: { item:req.body.menuid } } }
          ).then(result=>{
    ...
    app.post('/passw',(req,res,next)=>{
        ...
        bcrypt.hash(req.body.password,10)
    .then(hash =>{ 
        User.updateOne({_id:req.body.id },{password:hash}).then(result=>{
